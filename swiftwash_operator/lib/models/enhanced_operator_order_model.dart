...
class EnhancedOperatorOrderModel {
  final String id;
  ...
  factory EnhancedOperatorOrderModel.fromFirestore(DocumentSnapshot doc) {
    final data = doc.data() as Map<String, dynamic>;

    return EnhancedOperatorOrderModel(
      id: doc.id,
      ...
      estimatedCompletionTime: (data['estimatedCompletionTime'] as Timestamp?)?.toDate(),
  ...
  String get customerName => customerInfo?['name'] ?? 'Unknown Customer';
  String get customerPhone => customerInfo?['phone'] ?? 'N/A';

  bool get isActive => status.allowsCustomerActions && status != OrderStatus.cancelled;

  bool get canTrack => status.canTrack;
  bool get isUrgent => priority == 'urgent' || priority == 'high';
  bool get isOverdue => estimatedCompletionTime != null && DateTime.now().isAfter(estimatedCompletionTime!);
  ...
  // Get available actions for operators
  List<OrderStatus> get availableActions {
    if (status == OrderStatus.cancelled || status == OrderStatus.completed) {
      return [];
    }
    return status.nextPossibleStatuses;
  }
