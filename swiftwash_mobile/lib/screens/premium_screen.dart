...
class _PremiumScreenState extends State<PremiumScreen> {
  ...
  Future<void> _subscribeToPlan(SubscriptionPlan plan) async {
    ...
    try {
      ...
      // Process payment
      final paymentResult = await razorpayService.checkout(
        amount: (plan.price * 100).toInt(), // Convert to paisa
        name: 'SwiftWash Premium',
      ...
      if (paymentResult != null && paymentResult['success'] == true) {
        final paymentId = paymentResult['paymentId'] ?? 'premium_${DateTime.now().millisecondsSinceEpoch}';
        ...
        final errorMessage = paymentResult?['error'] ?? 'Payment was cancelled or failed.';
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
  ...
  Future<void> _renewSubscription(SubscriptionPlan plan) async {
    ...
    try {
      ...
      final paymentResult = await razorpayService.checkout(
        ...
        name: 'SwiftWash Premium Renewal',
      ...
      if (paymentResult != null && paymentResult['success'] == true) {
        final paymentId = paymentResult['paymentId'] ?? 'renewal_${DateTime.now().millisecondsSinceEpoch}';
