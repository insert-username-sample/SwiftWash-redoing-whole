...
import 'package:swiftwash_mobile/razorpay_service.dart';
...
class _Step4WidgetState extends State<Step4Widget> {
  ...
  void _handlePayment() async {
    if (_finalTotal <= 0) {
      ...
      final orderDocRef = await _saveOrder();
      if (!mounted) return;
      ...
      if (orderDocRef != null && mounted) {
        // Get the order document after it's saved
        final orderSnapshot = await orderDocRef.get();
        if (orderSnapshot.exists && mounted) {
          ...
          Navigator.of(context).pushReplacement(
            MaterialPageRoute(
              builder: (context) => TrackingScreen(orderId: orderSnapshot.id),
            ),
          );
        } else {
    ...
    final user = FirebaseAuth.instance.currentUser;
    final paymentResult = await _razorpayService.checkout(
      amount: (_finalTotal * 100).toInt(), // Amount in paise
      name: 'SwiftWash',
  ...
  Future<DocumentReference?> _saveOrder() async {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) return null;

    print('Saving order for user: ${user.uid}');

    // Get order ID based on location
    final orderId = await _generateOrderId(user.uid);

    final cartItems = await CartService.loadCart();

    // Determine the main service name from the order items
    String serviceName = 'Laundry Service'; // Default
    if (cartItems.isNotEmpty) {
      final firstItemServiceName = cartItems.first['serviceName'] as String? ?? '';
      if (firstItemServiceName.contains('Ironing')) {
        serviceName = 'Ironing Service';
      } else if (firstItemServiceName.contains('Swift')) {
        serviceName = 'Swift Laundry Service';
      } else if (firstItemServiceName.contains('Laundry')) {
    ...
    final Map<String, dynamic> order = {
      ...
      'swiftCharge': widget.swiftCharge,
      'discount': _discount,
      'finalTotal': _finalTotal,
      'createdAt': FieldValue.serverTimestamp(),
      'status': 'new', // Changed from 'Ongoing' to 'new' for consistency
      'serviceName': serviceName,
      'items': cartItems,
      // Add processing statuses for better tracking
      'currentProcessingStatus': 'sorting', // Initial processing status
      'processingStatuses': [], // Will be populated by operators
    };

    // Add selected address if available
    if (widget.selectedAddress != null) {
      order['address'] = widget.selectedAddress;
      print('Including selected address: ${widget.selectedAddress}');
    }

    final docRef = await FirebaseFirestore.instance.collection('orders').add(order);
    print('Order saved successfully with ID: $orderId, Address included: ${widget.selectedAddress != null}');
    return docRef;
  }

  Future<String> _generateOrderId(String userId) async {
    try {
      // Call server-side function to generate Order ID
      final callable = FirebaseFunctions.instance.httpsCallable('generateOrderId');
      final result = await callable.call(<String, dynamic>{'userId': userId});

      if (result.data['orderId'] != null) {
        String orderId = result.data['orderId'];

        // Add '-S' suffix for Swift orders
        final cartItems = await CartService.loadCart();
        final hasSwiftService = cartItems.any((item) => item['serviceType'] == 'swift');
        ...
        throw Exception(result.data['error'] ?? 'Failed to generate Order ID');
      }
    } catch (e) {
